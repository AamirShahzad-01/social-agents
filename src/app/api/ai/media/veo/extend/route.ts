...
// Extended timeout for video extension (5 minutes)
...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            const backendResponse = await fetch(`${PYTHON_BACKEND_URL}/api/v1/media/video/extend`, {
                method: 'POST',
                headers: {
            ...
            if (fetchError.name === 'AbortError') {
                return NextResponse.json(
                    {
                        success: false,
                        error: 'Request timed out. Video extension can take several minutes.'
                    },
                    { status: 408 }
                );
            }
        ...
        return NextResponse.json(
            { success: false, error: 'Failed to extend video' },
            { status: 500 }
        );
    }
}
