 ...
 * Generate video with first/last frame specification using Google Veo
 */
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            const backendResponse = await fetch(`${PYTHON_BACKEND_URL}/api/v1/media/video/frame-specific`, {
                method: 'POST',
                headers: {
        ...
        console.error('Veo frame-specific error:', error);
        return NextResponse.json(
            { success: false, error: 'Failed to generate frame-specific video' },
            { status: 500 }
        );
    }
}
