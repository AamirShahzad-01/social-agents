...
/**
 * POST /api/ai/media/kling/lip-sync
 * Synchronize lip movements with audio using Kling AI
 */
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            const backendResponse = await fetch(`${PYTHON_BACKEND_URL}/api/v1/media/kling/lip-sync`, {
                method: 'POST',
                headers: {
            ...
            if (fetchError.name === 'AbortError') {
                return createTimeoutResponse('Request timed out. Lip-sync processing can take several minutes.');
            }
            throw fetchError;
        }
    } catch (error) {
        console.error('Kling lip-sync error:', error);
        return createErrorResponse(error, 'Failed to process lip-sync');
    }
}
