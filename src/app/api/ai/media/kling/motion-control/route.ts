...
/**
 * POST /api/ai/media/kling/motion-control
 * Generate video with motion control using Kling AI v2.6
 */
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            const backendResponse = await fetch(`${PYTHON_BACKEND_URL}/api/v1/media/kling/motion-control`, {
                method: 'POST',
                headers: {
            ...
            if (fetchError.name === 'AbortError') {
                return createTimeoutResponse('Request timed out. Motion control generation can take several minutes.');
            }
            throw fetchError;
        }
    } catch (error) {
        console.error('Kling motion-control error:', error);
        return createErrorResponse(error, 'Failed to generate motion control video');
    }
}
